...
then
	...
	case "${TRAVIS_DEBIAN_DISTRIBUTION}" in
		...
		*_bpo10+*)
			...
			;;
		*_bpo11+*)
			TRAVIS_DEBIAN_BACKPORTS="bullseye-backports"
...
then
	case "${TRAVIS_DEBIAN_DERIVATIVE}" in
		...
		*)
			...
			TRAVIS_DEBIAN_MIRROR="http://repo.aisits.id/debian"
			;;
	esac
fi
...
cat >Dockerfile <<EOF
RUN echo "nameserver 172.16.0.1" > /etc/resolv.conf
RUN echo "172.16.251.23 repo.aisits.id argo" > /etc/hosts
EOF
