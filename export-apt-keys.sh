...
ssh devomd "mkdir -p /tb2/w3repo"
ssh omd "mkdir -p /tb2/w3repo"

LOCSIZE=$(ls -la ./trusted-keys | cut -d' ' -f5)
OMDSIZE=$(ssh devomd -C "ls -la /tb2/w3repo/trusted-keys | cut -d' ' -f5")
OMDSIZE2=$(( OMDSIZE * 2 ))

printf "\n\n SIZES: local=$LOCSIZE omd=$OMDSIZE \n"
if [[ $LOCSIZE -gt $OMDSIZE ]] && [[ $LOCSIZE -lt $OMDSIZE2 ]]; then
	scp ./trusted-keys devomd:/tb2/w3repo/
	scp ./trusted-keys omd:/tb2/w3repo/

	ssh devomd "chown webme.webme /tb2/w3repo/ /tb2/w3repo/phideb/ -R"
	ssh omd "chown webme.webme /tb2/w3repo/ /tb2/w3repo/phideb/ -R"
fi
